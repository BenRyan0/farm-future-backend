...
class listingController {
  ...
  calculateTotalPrice = (pricePerUnit, yieldAmount, priceUnit, yieldUnit) => {
    const priceUnitGroup = getUnitGroup(priceUnit);
    const yieldUnitGroup = getUnitGroup(yieldUnit);
    ...
    const priceUnitMultiplier = unitConversionMap[priceUnit] || 1; // Default multiplier 1 if not found
    const yieldUnitMultiplier = unitConversionMap[yieldUnit] || 1; // Default multiplier 1 if not found
    ...
    // Calculate perYield
    const perYield = yieldAmount / pricePerUnit;
  
    return { totalPrice, perYield };
  };
...
add_listing = async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Validate that we have at least two images
    ...
    // Validate price and expected yield as numbers
    const parsedPrice = parseFloat(price);
    const parsedExpectedHarvestYield = parseFloat(expectedHarvestYield);

    if (isNaN(parsedPrice) || isNaN(parsedExpectedHarvestYield)) {
      return responseReturn(res, 400, { error: "Price and expected harvest yield must be valid numbers." });
    }
    ...
    try {
      let allImageUrl = [];
      for (let i = 0; i < images.length; i++) {
        const resizedImagePath = await resizeImage(images[i].filepath);
        const result = await cloudinary.uploader.upload(resizedImagePath, {
          folder: "listings",
        });
      ...
      // Calculate the total price and perYield using calculateTotalPrice
      let totalPrice, perYield;
      try {
        ({ totalPrice, perYield } = calculateTotalPrice(
          parsedPrice,
          parsedExpectedHarvestYield,
          unit,
          yieldUnit
      ...
      const listing = await listingModel.create({
        ...
        images: allImageUrl,
        totalPrice,
        perYield, // Add perYield to the listing
        locationInfo,
        additionalLocationInfo,
        discount,
        mapsLink
